...
public class HttpUtil {
    ...
    private CloseableHttpClient createClient() throws GeneralSecurityException {
        try {
            ...
            log.log(Level.WARNING, "browser-task >> HttpUtil >> getRequestFactory >> Exception:", ex);
    ...
    public ResponseEntity<String> sendRequest(String path, HttpUriRequestBase request, HttpHeaders headers) {
        try (CloseableHttpClient client = createClient()) {
            ...
            log.log(Level.WARNING, MessageFormat.format("browser-task >> HttpUtil >> sendRequest >> url: {0} >> Exception:", path), e);
            throw e;
        }
    }
